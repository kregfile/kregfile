"use strict";

const {DistributedTracking} = require("./broker/collections");

const acctFloods = new DistributedTracking("acctfloods");
const clients = new DistributedTracking("clients");
const floods = new DistributedTracking("floods");
const roomFloods = new DistributedTracking("roomfloods");
const uploadFloods = new DistributedTracking("ufloods");


class FloodProtector {
  constructor(what, where, max, when) {
    this.what = what;
    this.where = where;
    this.max = max;
    this.when = when;
    this.active = false;
    this.end = 0;
    Object.seal(this);
  }

  get() {
    return this.where.get(this.what);
  }

  get isFlooding() {
    if (this.active) {
      return this.end;
    }
    return this.get() > this.max;
  }

  async flooding() {
    if (this.active) {
      return this.end;
    }
    let cur = this.get();
    if (cur > this.max) {
      // coming from another websocket, so estimate
      this.active = true;
      this.end = this.when + Date.now();
      setTimeout(() => {
        this.end = 0;
        this.active = false;
      }, this.when);
      return this.end;
    }
    cur = await this.where.incr(this.what);
    if (cur === 1) {
      this.end = this.when + Date.now();
      setTimeout(() => {
        this.where.delete(this.what);
        this.end = 0;
        this.active = false;
      }, this.when);
    }
    if (cur <= this.max) {
      return false;
    }
    this.active = true;
    return this.end;
  }

  async delete() {
    await this.where.delete(this.what);
  }
}

module.exports = {
  FloodProtector,
  acctFloods,
  clients,
  floods,
  roomFloods,
  uploadFloods,
};
