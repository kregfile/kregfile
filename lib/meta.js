...
async function generateAssetsPdf(storage) {
  ...
  const prev = await gm(pdfBuffer)
    .selectFrame(0) 
    .size()
    .toBuffer('jpg',async function (err, prev) {
      
      const base = await sharp(prev).
...
async function generateAssetsAudio(storage) {
  ...
  if (JAIL) {
    ...
    exiftool = [
      ...
      EXIFTOOL, "-b", "-Picture", `${p.dir}/${p.base}`
    ];
  }
  else {
...
async function generateAssets(storage) {
  const {mime} = storage;
  console.log('storage.meta.type', storage.meta.type);
  if (storage.meta.type == "PDF" ||storage.meta.type == "pdf") {
  ...
  try {
    ...
    if (JAIL) {
      ...
      filetool = [
        "firejail",
        "--quiet", `--profile=${PROFILE}`, `--private=${p.dir}`,
        FILETOOL, "-d", "--mime-type", `${p.dir}/${p.base}`
      ];
    }
    else {
