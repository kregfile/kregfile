...
const zscore = promisify(BROKER.PUB.zscore.bind(BROKER.PUB));
const zrevrank = promisify(BROKER.PUB.zrevrank.bind(BROKER.PUB));
const zincrby = promisify(BROKER.PUB.zincrby.bind(BROKER.PUB));
...
class User {
  ...
  async uploadStats() {
    ...
    try {
      const fu = await zscore(UPSTAT_FILES, this.account);
      if (!fu) {
        return rv;
      }
      rv.files = parseInt(fu, 10) || 0;
      rv.filesRank = await zrevrank(UPSTAT_FILES, this.account) + 1;
      rv.uploaded = parseInt(await zscore(UPSTAT_BYTES, this.account), 10) || 0;
      rv.uploadedRank = await zrevrank(UPSTAT_BYTES, this.account) + 1;
      return rv;
    }
    finally {
  ...
  async addUpload(bytes) {
    await zincrby(UPSTAT_FILES, 1, this.account);
    await zincrby(UPSTAT_BYTES, bytes | 0, this.account);
  ...
  static async getInfo(acct) {
    ...
    return await (new User(JSON.parse(data))).getInfo();
  }
