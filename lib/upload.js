...
class StorageLocation {
  ...
  async _addAsset(asset) {
    try {
      ...
      const full = this.full + asset.ext;
      const {data} = asset;
...
async function realupload(req, res) {
  try {
    ...
    try {
      ...
      res.json({key});
      if (storage) {
        generateAssets(newStorage).then(console.log).catch(console.error);
      }
    }
    catch (ex) {
      if (!ex.retryable) {
...
const serveStatic = ss(UPLOADS_DIR, serveOpts);
...
async function serve(req, res, next) {
  ...
  try {
    ...
    req[U] = u;
    req[S] = storage;
    req.originalUrl = req.url = storage.url;

    if (a) {
      a = storage.assets.get(a);
      if (!a) {
        throw new Error("Invalid asset");
      }
      req[A] = a;
      req.originalUrl = req.url = storage.url + a.ext;
    }
