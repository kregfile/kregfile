...
const {CoalescedUpdate, debounce, sleep, toMessage} = require("./util");
...
class FileLister {
  constructor(room) {
    ...
    this.onadded = new CoalescedUpdate(1000, files => {
      console.warn("pushing", files.map(e => e.toString()));
      files.forEach(e => {
  ...
  onfile(action, file) {
    console.warn("got", action, file.toString());
    if (action === "add") {
