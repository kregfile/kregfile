...
const redis = BROKER.getMethods(
  "del", "expire", "ttl", "rpushex", "rpushx", "lrange",
);

const QUEUE = new class Queue {
  key(client, stoken) {
    return `mrq:${client.ip}:${client.roomid}:${client.role}:${stoken}`;
  }

  async add(client) {
    const k = this.key(client, client.stoken);
    // push the sential
    await redis.rpushex(k, msgpack.encode({
      seq: -1
    }).toString("binary"), 45);
  }
  ...
  async flush(client) {
    ...
    await this.add(client);
  }

  async push(client, data) {
    const k = this.key(client, client.stoken);
    const pushed = await redis.rpushx(
      k, msgpack.encode(data).toString("binary"));
    return pushed > 0;
  }

  async get(client, stoken, rseq) {
    const k = this.key(client, stoken);
    const l = await redis.lrange(k, 0, -1);
    if (!l || !l.length) {
      return null;
    }
    return l.map(e => msgpack.decode(Buffer.from(e, "binary"))).
      filter(e => e.seq > rseq);
  }

  async alive(client) {
    const k = this.key(client, client.stoken);
    const l = await redis.ttl(k);
    return l > 0;
  }
}();
...
class Client {
  constructor(room, socket, rtoken) {
    ...
    socket.conn.on("packet", this.onpacket);
  ...
  onpacket(p) {
    ...
    QUEUE.flush(this).catch(console.error);
  }

  async oncontinue(stoken, seq) {
    try {
      const queue = await QUEUE.get(this, stoken, seq);
      if (queue === null) {
        this.emitOnce(`continue-${stoken}`, false);
        return;
      }
      queue.forEach(e => this.emit(e.ev, ...e.args));
    }
    catch (ex) {
      console.error("failed to re-emit queued", ex);
    }
    finally {
      await QUEUE.delete(this, stoken, seq);
    }
  ...
  onclose() {
    ...
    this.socket.conn.removeListener("packet", this.onpacket);
    ...
    this.socket = null;
    QUEUE.add(this).catch(console.error);
    DYING.add(this);
  ...
  onreallyclose() {
    ...
    this.room.unref();
    QUEUE.delete(this).catch(console.error);
