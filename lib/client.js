...
class Client {
  ...
  broadcast(msg) {
    ...
    BROKER.emit(`${this.roomid}:message`, Object.assign({
      admin,
    }, msg));
  }
  ...
  async onprofileinfo(profile) {
    try {
      ...
      if (user.gravatar) {
        rv.gravatar = user.gravatar;
      }
      this.emit(`profileinfo-${profile}`, rv);
    }
    catch (ex) {
  ...
  static async create(socket, token) {
    const client = new Client(socket, token);
    await client.init();
    return client;
  }
}
