...
class Room extends EventEmitter {
  ...
  static async create(ip, user, rtoken) {
    ...
    if (!isMod) {
      ...
      const anyBans = await Promise.all(["mute", "upload", "hellban"].map(b => bans.findBan(b, ip, user && user.account)));
      if (anyBans.some(e => !!e)) {
    ...
    if (CONFIG.get("roomCreationRequiresAccount") && !user) {
      throw new Error("Anonymous room creation is disabled on this server");
    }
