...
const parser = require("../common/sioparser");
...
app.get("/adiscover", async (req, res, next) => {
  if (!req.user || !req.user.role === "mod") {
    next();
    return;
  }
...
app.get("/modlog/revert/:id", async (req, res, next) => {
  if (!req.user || !req.user.role === "mod") {
    next();
    return;
  }
...
app.get("/modlog/:id", async (req, res, next) => {
  if (!req.user || !req.user.role === "mod") {
    next();
    return;
  }
...
app.get("/modlog", async (req, res, next) => {
  if (!req.user || !req.user.role === "mod") {
    next();
    return;
  }
